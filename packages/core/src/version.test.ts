...
test('same code different formatting produces different metadata', async t => {
  async function getBytecodeByContractName(
    contractName: string,
  ): Promise<string> {
    const solcOutput = JSON.parse(
      await fs.readFile('cache/solc-output.json', 'utf8'),
    );
    return solcOutput.contracts['contracts/test/Version.sol'][contractName].evm
      .bytecode.object;
  }

  const GreeterBytecode = await getBytecodeByContractName('Greeter');
  const GreeterDifferentFormattingBytecode = await getBytecodeByContractName(
    'GreeterDifferentFormatting',
  );

  t.not(
    hashBytecode(GreeterBytecode),
    hashBytecode(GreeterDifferentFormattingBytecode),
  );
  t.is(
    hashBytecodeWithoutMetadata(GreeterBytecode),
    hashBytecodeWithoutMetadata(GreeterDifferentFormattingBytecode),
  );
});
...
test('removes metadata from Solidity 0.4 bytecode', t => {
  ...
  t.is(
    hashBytecodeWithoutMetadata(bytecode),
    hashBytecode(`0x${contractBytecode}`),
  );
});
