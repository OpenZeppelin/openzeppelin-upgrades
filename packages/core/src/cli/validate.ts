...
import { ValidationError, convertToWarning, errorKinds } from '../validate/run';
...
Options:
  ...
  --unsafeAllow "<VALIDATION_ERROR>[,<VALIDATION_ERROR>...]"  Selectively disable one or more validation errors or warnings. Comma-separated list with one or more of the following:
      Errors: ${errorKinds.filter(kind => !convertToWarning.includes(kind)).join(', ')}
      Warnings: ${convertToWarning.join(', ')}
