    ...
    `file.sol:1: Upgraded \`s1\` to an incompatible type␊
      ...
      - In struct StorageUpgrade_FunctionType_Visibility_V2_Bad.S␊
        - Upgraded \`m\` to an incompatible type␊
          - Bad upgrade visibility changed from internal to external`

## function mutability change

> Snapshot 1

    `file.sol:1: Upgraded \`s1\` to an incompatible type␊
      - Bad upgrade from struct StorageUpgrade_FunctionType_Mutability_V1.S to struct StorageUpgrade_FunctionType_Mutability_V2_Bad.S␊
      - In struct StorageUpgrade_FunctionType_Mutability_V2_Bad.S␊
        - Upgraded \`m\` to an incompatible type␊
          - Bad upgrade mutability changed from view to nonpayable`
