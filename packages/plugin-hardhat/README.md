...
> **Note:** Hardhat 3 supports both ethers and viem. This plugin uses `@nomicfoundation/hardhat-ethers` internally and loads it automatically. If your project uses viem, use the viem-based API from `@openzeppelin/hardhat-upgrades/viem`: its functions take contract names and return viem contract instances, so your scripts and tests never need to touch ethers. See [Usage with viem](#usage-with-viem).
...
Register both plugins in your [`hardhat.config.ts`](https://hardhat.org/config/):
...
> **Note:** The viem-based API does not include the OpenZeppelin Defender functions.
