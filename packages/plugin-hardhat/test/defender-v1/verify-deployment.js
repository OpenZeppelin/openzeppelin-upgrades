...
const greeterV3DeployedBytecode =
  '';
...
test.beforeEach(async t => {
  ...
  const verifyDeploymentModule = proxyquire('../../dist/defender-v1/verify-deployment', {
    './utils': {
...
test('verifies deployment uploading artifact', async t => {
  ...
  const result = await verifyDeployment(address, 'GreeterDefenderV3', artifactUri);

  t.is(result.matchType, verificationResult.matchType);
  sinon.assert.calledWithExactly(fakeClient.verifyDeployment, {
    contractAddress: address,
    contractName: 'GreeterDefenderV3',
    solidityFilePath: 'contracts/GreeterDefender.sol',
    referenceUri: artifactUri,
    contractNetwork: t.context.fakeChainId,
    artifactPayload: JSON.stringify({
      ...require('../../artifacts/contracts/GreeterDefender.sol/GreeterDefenderV3.json'),
...
test('verifies deploy with uploaded artifact', async t => {
  ...
  const result = await verifyDeploymentWithUploadedArtifact(address, 'GreeterDefenderV3', artifactUri);
...
test('returns extended artifact for verification', async t => {
  ...
  const result = await getVerifyDeployArtifact('GreeterDefenderV3');

  t.is(
    result.bytecode,
    '',
  );
  t.is(result.deployedBytecode, `0x${greeterV3DeployedBytecode}`);

  // See artifacts/build-info/BUILD.json.output.contracts.'contracts/GreeterDefender.sol'.GreeterDefenderV3.evm.deployedBytecode
...
test('returns build info for verification', async t => {
  ...
  const result = await getVerifyDeployBuildInfo('GreeterDefenderV3');

  t.is(
    result.output.contracts['contracts/GreeterDefender.sol']['GreeterDefenderV3'].evm.deployedBytecode.object,
...
test('returns bytecode digest', async t => {
  ...
  const result = await getBytecodeDigest('GreeterDefenderV3');
