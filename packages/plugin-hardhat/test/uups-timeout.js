...
test('timeout too low, long polling', async t => {
  const error = await t.throwsAsync(() =>
    upgrades.deployProxy(t.context.Greeter, ['Hello, Hardhat!'], { kind: 'uups', timeout: 1, pollingInterval: 1000 }),
  );
...
test('timeout too low, short polling', async t => {
  const error = await t.throwsAsync(() =>
    upgrades.deployProxy(t.context.Greeter, ['Hello, Hardhat!'], { kind: 'uups', timeout: 1, pollingInterval: 1 }),
  );
  t.true(error.message.includes(TIMED_OUT) && error.message.includes(USE_OPTIONS));
});
...
test('timeout too low, 0 ms polling', async t => {
  const error = await t.throwsAsync(() =>
    upgrades.deployProxy(t.context.Greeter, ['Hello, Hardhat!'], { kind: 'uups', timeout: 1, pollingInterval: 0 }),
  );
  t.true(error.message.includes(TIMED_OUT) && error.message.includes(USE_OPTIONS));
});
...
test('infinite timeout, short polling', async t => {
  await upgrades.deployProxy(t.context.Greeter, ['Hello, Hardhat!'], { kind: 'uups', timeout: 0, pollingInterval: 10 });
});

test('infinite timeout, 0 ms polling', async t => {
  await upgrades.deployProxy(t.context.Greeter, ['Hello, Hardhat!'], { kind: 'uups', timeout: 0, pollingInterval: 0 });
});

test('single option', async t => {
  await upgrades.deployProxy(t.context.Greeter, ['Hello, Hardhat!'], { kind: 'uups', pollingInterval: 0 });
});

test('upgrade', async t => {
  // automine to immediately deploy a new proxy to use in below tests
  await network.provider.send('evm_setAutomine', [true]);
  const proxy = await upgrades.deployProxy(t.context.Greeter, ['Hello, Hardhat!'], {
    kind: 'uups',
    timeout: 0,
    pollingInterval: 0,
  });
  ...
  const error = await t.throwsAsync(() =>
    upgrades.upgradeProxy(proxy, t.context.GreeterV2, { timeout: 1, pollingInterval: 1 }),
  );
  t.true(error.message.includes(TIMED_OUT) && error.message.includes(USE_OPTIONS));

  // upgrade: infinite timeout
  await upgrades.upgradeProxy(proxy, t.context.GreeterV2, { timeout: 0, pollingInterval: 10 });
});
