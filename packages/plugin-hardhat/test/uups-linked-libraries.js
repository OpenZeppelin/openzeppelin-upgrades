...
test('without flag', async t => {
  ...
  const Token = await getLinkedContractFactory('TokenProxiable', { SafeMath: safeMathLib.address });
  await t.throwsAsync(
...
test('with flag', async t => {
  ...
  const TokenNew = await getLinkedContractFactory('TokenProxiable', { SafeMath: safeMathLib2.address });
