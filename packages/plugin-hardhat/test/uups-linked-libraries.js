...
test('without flag', async t => {
  ...
  const Token = await getLinkedContractFactory('TokenProxiable', { SafeMath: await safeMathLib.getAddress() });
  await t.throwsAsync(
    () => upgrades.deployProxy(Token, ['TKN', 10000], { kind: 'uups' }),
    undefined,
...
test('with flag', async t => {
  ...
  const TokenNew = await getLinkedContractFactory('TokenProxiable', { SafeMath: await safeMathLib2.getAddress() });
  ...
  const TokenV2 = await getLinkedContractFactory('TokenV2Proxiable', { SafeMath: await safeMathLib.getAddress() });
  ...
  const TokenV2New = await getLinkedContractFactory('TokenV2Proxiable', { SafeMath: await safeMathLib2.getAddress() });
  ...
  const TokenV3 = await getLinkedContractFactory('TokenV3Proxiable', {
