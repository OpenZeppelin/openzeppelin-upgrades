...
test('upgradeProxy', async t => {
  ...
  const portfolio = await upgrades.deployProxy(Portfolio, [], { kind: 'uups' });
  const portfolio2 = await upgrades.upgradeProxy(portfolio.address, PortfolioV2, { kind: 'uups' });
...
test('upgradeProxy with incompatible layout', async t => {
  ...
  const portfolio = await upgrades.deployProxy(Portfolio, [], { kind: 'uups' });
  const error = await t.throwsAsync(() => upgrades.upgradeProxy(portfolio.address, PortfolioV2Bad, { kind: 'uups' }));
