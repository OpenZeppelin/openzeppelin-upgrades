...
/**
 * Deployment script for TokenV1 with UUPS proxy pattern.
 *
 * This script demonstrates how to deploy an upgradeable contract using
 * Hardhat 3's connection-based API with the UUPS proxy pattern.
 *
 * Key differences from Transparent proxy:
 * - No ProxyAdmin contract is deployed
 * - Upgrade authorization is handled by the implementation contract itself
 * - More gas-efficient for upgrades
 */
async function main() {
  ...
  // With UUPS, the owner can upgrade via _authorizeUpgrade
  console.log('\nDeploying TokenV1 with UUPS proxy...');
  ...
  console.log('\n📝 Note: With UUPS, upgrades are authorized by the contract itself');
  console.log('   The owner (deployer) can upgrade via _authorizeUpgrade function');
}
