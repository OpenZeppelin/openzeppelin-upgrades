...
To create a storage gap, declare a fixed-size array in the base contract with an initial number of slots. This can be an array of `uint256` so that each element reserves a 32 byte slot. Use the naming convention `__gaps` so that OpenZeppelin Upgrades will recognize the gap:
