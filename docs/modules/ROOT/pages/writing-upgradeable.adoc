...
This convention involves placing all storage variables of a contract into one or more structs and annotating those structs with `@custom:storage-location erc7201:<NAMESPACE_ID>`.  A namespace id is a string that uniquely identifies each namespace in a contract, and the same id must not be defined more than once in a contract or any of its base contracts.
...
WARNING: Solidity version 0.8.20 or higher is recommended when using namespaced storage layouts. Using a lower version of the Solidity compiler will cause the `@custom:storage-location` annotations to be ignored, which results in upgrade safety checks being skipped for the namespaces.