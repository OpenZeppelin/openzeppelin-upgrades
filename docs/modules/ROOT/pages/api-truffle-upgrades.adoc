...
* `kind`: (`"uups" | "transparent"`) Choose between a UUPS or Transparent proxy. Defaults to Transparent. See xref:contracts:api:proxy.adoc#transparent-vs-uups[Transparent vs UUPS]. Not applicable for Beacon proxies except when xref:#import-proxy[importing].
...
Imports an existing proxy or beacon deployment to be used with this plugin. Provide the address of an existing proxy or beacon and the Truffle contract class of the implementation contract that was deployed. Use this function to recreate a lost https://docs.openzeppelin.com/upgrades-plugins/1.x/network-files[network file] by importing previous deployments, or to register proxies or beacons for upgrading even if they were not originally deployed by this plugin. Supported for UUPS, Transparent, and Beacon proxies, as well as beacons.

* `force`: whether to force the import even if the runtime bytecode at the implementation address does not correspond to the provided Truffle contract class. This may be needed if the implementation was compiled with different compiler settings.
* `kind`: (`"uups" | "transparent" | "beacon"`) if importing from a proxy address where the kind of proxy cannot be determined, use this option to specify whether the proxy is a UUPS, Transparent, or Beacon proxy. This may be needed if the proxy contract was compiled with different compiler settings.
