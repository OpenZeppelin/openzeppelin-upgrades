...
The plugins support the UUPS, transparent, and beacon proxy patterns. UUPS and transparent proxies are upgraded separately, whereas a number of beacon proxies can be upgraded together by upgrading the beacon that they point to. For more details on the different proxy patterns available, see the documentation for https://docs.openzeppelin.com/contracts/4.x/api/proxy[Proxies].
