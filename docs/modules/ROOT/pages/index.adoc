...
The plugins support the UUPS, transparent, and beacon proxy patterns. UUPS and transparent proxies are upgraded independently of other proxies, whereas a number of beacon proxies can be upgraded together by upgrading the beacon that they point to. For more details on the different proxy patterns available, see the documentation for https://docs.openzeppelin.com/contracts/4.x/api/proxy[Proxies]. 

[[how-plugins-work]]
== How the plugins work

Both plugins provide functions which take care of managing upgradeable deployments of your contracts. In the case of UUPS and transparent proxies, these are `deployProxy` and `upgradeProxy`. For beacon proxies, these are `deployBeacon`, `deployBeaconProxy`, and `upgradeBeacon`.
