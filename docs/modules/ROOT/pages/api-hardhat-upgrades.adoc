...
** `proxyFactory`: (`(hre, signer) => Promise<ContractFactory>`) customizes Proxy contract factory to use, allowing a different proxy implementation. Defaults to the functions defined in `packages/plugin-hardhat/src/utils/factories.ts` for each kind of proxy.
*** *Since:* `@openzeppelin/hardhat-upgrades@3.7.0`
** `deployFunction`: (`(hre, opts, factory, ...args) => Promise<EthersOrDefenderDeployment>`) customizes the function used to deploy the Proxy. Defaults to the `deploy` function defined in `packages/plugin-hardhat/src/utils/deploy.ts`.
*** *Since:* `@openzeppelin/hardhat-upgrades@3.7.0`
