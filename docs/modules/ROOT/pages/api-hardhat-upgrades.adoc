...
* `getTxResponse`: if set to `true`, causes this function to return an ethers transaction response if the new implementation contract was deployed during this function call. Note this function will only return an address if the new implementation contract was already previously deployed.
* See <<common-options>>.
...
Returns an object with the URL of the Defender proposal, and the ethers transaction response if the new implementation contract was deployed during this function call. Note that the ethers transaction response will be undefined if the new implementation contract was already previously deployed.
