...
Upgrades a https://docs.openzeppelin.com/contracts/4.x/api/proxy#UpgradeableBeacon[beacon] at a specified address to a new implementation contract, and returns the beacon contract instance.
...
async function deployBeaconProxy(
  beaconAddress: string,
  ContractOrSigner: ethers.ContractFactory | ethers.Signer,
  args: unknown[] = [],
  opts: {
    initializer: string | false,
  } = {},
...
Given a previous proxy contract instance or a combination of proxy address and signer, returns a contract instance with the proxy address and its current implementation interface. Only supported for beacon proxies.
