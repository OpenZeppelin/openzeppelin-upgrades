...
** `"struct-internal-function"`: Allow internal functions in structs. Internal functions are code pointers which will no longer be valid after an upgrade, so they must be reassigned during upgrades.
