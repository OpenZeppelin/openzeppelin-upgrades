...
* `platform`: Deploy contracts using the OpenZeppelin Platform instead of ethers.js. See xref:platform-upgrades.adoc[Using with OpenZeppelin Platform].
...
** `platform`: if calling this function using `platform.deployContract`, this option is not needed. If calling this function using `upgrades.deployContract`, this option must be set to `true`.
...
Same as <<defender-propose-upgrade>> but supports deploying the new implementation contract using the OpenZeppelin Platform if this function is called as `platform.proposeUpgrade` or if the `platform` option is set to `true`. Requires configuring an OpenZeppelin Platform Team API Key.
