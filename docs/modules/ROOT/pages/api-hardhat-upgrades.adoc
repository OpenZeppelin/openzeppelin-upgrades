...
* `proxyFactory`: (`(hre, signer) => Promise<ContractFactory>`) customizes Proxy contract factory to use, allowing a different proxy implementation. See https://github.com/OpenZeppelin/openzeppelin-upgrades/blob/master/packages/plugin-hardhat/src/utils/factories.ts[factories.ts] for the default factory for each kind of proxy.
