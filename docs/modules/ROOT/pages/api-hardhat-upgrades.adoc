...
** `platform`: to use this function, you must enable OpenZeppelin Platform deployments using any of the ways described in xref:platform-upgrades.adoc[Using with OpenZeppelin Platform] in order to set this to `true`. This function does not support `false`.
...
Same as <<defender-propose-upgrade>> but supports deploying the new implementation contract using the OpenZeppelin Platform. See xref:platform-upgrades.adoc[Using with OpenZeppelin Platform].
