...
await upgrades.validateUpgrade(PROXY_ADDRESS, GreeterV2);
----
...
Deploys a https://docs.openzeppelin.com/contracts/4.x/api/proxy#ProxyAdmin[proxy admin] contract and returns its address if one was not already deployed on the current network, or just returns the address of the proxy admin if one was already deployed. Note that the upgrades plugins currently only supports using one proxy admin per network.
