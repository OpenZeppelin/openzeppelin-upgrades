...
* `signer`: an ethers signer to use for deploying the beacon proxy. If not specified, uses the signer from the contractFactory option if available, otherwise uses the https://hardhat.org/plugins/nomiclabs-hardhat-ethers.html#helpers[default signer from hardhat-ethers].
