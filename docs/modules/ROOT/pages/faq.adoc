...
Deployment and upgrade related function come with an optional `opts` object, which includes a an `unsafeAllow` option. This can be set to disable any check performed by the plugin. The list of errors is the following types that can individually be disabled is:
...
For example, in order to upgrade to an implementation that contains a delegate call, you call do
...
Additionnally, it is possible to preciselly disable checks directly from the solidity source code using natspec comments. This requiers solidity >=0.8.2.
